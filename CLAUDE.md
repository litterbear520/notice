...
- 品牌视觉是 Fable 蝴蝶主题（用户指定，参照 Claude Fable 5 官方蝴蝶视觉）：`app/components/Fable.tsx` 提供 `Butterfly`（导航/登录 logo）与 `FableFlight`（首页时间线上方五只蝴蝶沿虚线飞行），配色走 CSS 令牌、动画受全局 `prefers-reduced-motion` 约束
