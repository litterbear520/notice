...
- 内置两个开箱即用的源：
  ...
  - **火山引擎公告目录页**：`https://docs.volcengine.com/docs/82379/1159176?lang=zh`（无 RSS，文档中心目录页，按年份分「模型发布公告 / 模型下线公告 / 产品更新公告」三类，需专用抓取器）
...
| source_id | 所属源，外键 |
| title | 标题 |
| url | 原文链接，**(source_id, url) 唯一**——既是去重键，也是 webpage 链接差异检测的「已见过」判断依据 |
...
- **volcengine**：专用抓取器。抓取公告目录页，解析「模型发布公告 / 模型下线公告 / 产品更新公告」分类下的文档链接和标题（含历年归档）。published_at 页面上无法获取，用首次抓取时间。实现注意：该页面初步验证为服务端渲染可直接解析；若实现时发现关键内容为 JS 渲染，改用其文档树 JSON 接口（浏览器开发者工具中确认具体接口）。
...
- **适配器解析**：用保存的真实样本（阿里云 RSS XML、火山公告页 HTML、通用网页 HTML）做 fixture，断言解析出的条目结构。
